      ...
      <div class="col-sm-6 offset-3">
              ...
              <div v-for="message in messages" :key="message.id" class="row chat-section">
                <template v-if="username === message.user.username">
                  <div class="col-sm-7 offset-3">
                    <span class="card-text speech-bubble speech-bubble-user
                          float-right text-white subtle-blue-gradient">
                      {{ message.message }}
                    </span>
                  </div>
                  <div class="col-sm-2">
        ...
        <div v-else>
          ...
          <button @click="startChatSession"
                  class="btn btn-primary btn-lg btn-block">Start Chatting</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    startChatSession() {
      ...
      this.axios.post(url)
        .then((response) => {
          ...
          self.$router.push(`/chats/${response.data.uri}/`);
        })
        .catch((response) => {
    ...
    postMessage(event) {
      ...
      const url = `http://localhost:8000/api/chats/${this.$route.params.uri}/messages/`;
    ...
    joinChatSession() {
      ...
      const url = `http://localhost:8000/api/chats/${uri}/`;
    ...
    fetchChatSessionHistory() {
      const url = `http://127.0.0.1:8000/api/chats/${this.$route.params.uri}/messages/`;
      this.axios.get(url)
        .then((response) => {
